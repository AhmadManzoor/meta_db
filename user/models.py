...
class AbstractUserModelManager(BaseUserManager):
    ...
    def brand_active_user(self, brand_id=None):
        return super(AbstractUserModelManager, self).get_queryset().filter(brand_id=brand_id, _is_active='Y', _is_approved='Y')

    def os_user(self):
        return super(AbstractUserModelManager, self).get_queryset().filter(brand_id__isnull=True)

    def os_active_user(self):
        return super(AbstractUserModelManager, self).get_queryset().filter(brand_id__isnull=True, _is_active='Y', _is_approved='Y')
...
class AbstractUserModel(AbstractBaseUser):
    ...
    register_source = models.CharField(max_length=10, db_column='TBWhichPlace', blank=True, default='WEB',
                                       help_text='mark source of register')
    customer_id = models.CharField(max_length=8, db_column='TBCustomer_ID', blank=True, unique=True,
    ...
    brand_id = models.CharField(max_length=3, db_column='onVendor_ID', blank=True, null=True,
    ...
    ct_address1 = models.CharField(max_length=200, db_column='CTAddress1', blank=True,
                                   help_text='company address 1')
    ct_address2 = models.CharField(max_length=200, db_column='CTAddress2', blank=True,
                                   help_text='company address 2')
    ct_city = models.CharField(max_length=100, db_column='CTCity', blank=True,
                               help_text='company city')
    ct_state = models.CharField(max_length=100, db_column='CTState', blank=True,
                                help_text='company state')
    ct_zipcode = models.CharField(max_length=100, db_column='CTZip', blank=True,
                              help_text='company zip')
    ct_country = models.CharField(max_length=100, db_column='CTCountry', blank=True, default='US',
    ...
    mail_address1 = models.CharField(max_length=200, db_column='CTMailAddress1', blank=True,
                                     help_text='shipping address 1')
    mail_address2 = models.CharField(max_length=200, db_column='CTMailAddress2', blank=True,
                                     help_text='shipping address 2')
    mail_city = models.CharField(max_length=100, db_column='CTMailCity', blank=True,
                                 help_text='shipping city')
    mail_state = models.CharField(max_length=100, db_column='CTMailState', blank=True,
                                  help_text='shipping state')
    mail_zipcode = models.CharField(max_length=100, db_column='CTMailZip', blank=True,
                                help_text='shipping zipcode')
    mail_country = models.CharField(max_length=100, db_column='CTMailCountry', blank=True,
                                    help_text='shipping country')
    mail_phone_number = models.CharField(max_length=100, db_column='CTMailPhone', blank=True,
    ...
    total_order_amount = models.DecimalField(decimal_places=4, null=True, max_digits=19, db_column='TotalOrderAmount',
    ...
    _is_approved = models.CharField(max_length=1, db_column='Approved', default='N',
    ...
    is_available_email = models.BooleanField(null=True, db_column='isAvailableEmail', blank=True, default=True)
    is_email_verified = models.BooleanField(db_column='IsEmailVerified', blank=True, null=True, default=False)
    _is_staff = models.CharField(max_length=1, db_column='nOSStaff', blank=True, default='N', help_text='check staff')
    store_credit_amount = models.DecimalField(decimal_places=4, max_digits=19, db_column='CTStoreCredit',
                                              help_text='store credit', default=0.00)
    auth_password = models.CharField(max_length=100, db_column='AuthPassword', blank=True,
                                     help_text='uuid as a token to password reset')
    auth_password_on_date = models.DateTimeField(null=True, db_column='AuthPasswordOnDate', blank=True,
                                                 help_text='requested date and time')
    changed_password_on_date = models.DateTimeField(null=True, db_column='ChangedPasswordOnDate', blank=True,
                                                    help_text='date and time of changed password')
    _mobile_phone_validation = models.CharField(max_length=1, db_column='Confirm1st', blank=True, help_text='mobile phone number validation', default='N')
    ...
    seller_permit_image = models.CharField(max_length=200, db_column='CTPermitImg1', blank=True, help_text='user seller permit image')
    seller_permit_image2 = models.CharField(max_length=200, db_column='CTPermitImg2', blank=True, help_text='user seller permit image')
    ...
    # favoritestbvendor = models.ForeignKey(Tbvendor, null=True, db_column='FavoritesTBVendor_ID', blank=True)
...
class UserModel(AbstractUserModel):
    job_title = models.ForeignKey('JobTitleModel', null=True, db_column='TBCustomerJobTitle_ID', blank=True, default=1, on_delete=models.DO_NOTHING)
...
class AbstractUserShippingAddressModel(models.Model):
    ...
    for_brand_web = models.CharField(max_length=3, db_column='onVendor_ID',
                                     blank=True, help_text='brand id for brand web site orders')
    ...
    _is_default = models.CharField(max_length=1, db_column='nDefault', default='N',
                                   help_text='default address')
...
class UserShippingAddressModel(AbstractUserShippingAddressModel):
    user = models.ForeignKey('UserModel', db_column='TBCustomer_ID', to_field='customer_id', related_name='shipping_addresses', on_delete=models.DO_NOTHING)
...
# COUNTRY LIST
class AbstractCountryListModel(models.Model):
    """
    READ ONLY MODEL
    """
    ...
    is_active = models.BooleanField(null=True, db_column='Active', default=True)
...
class CountryListModel(AbstractCountryListModel):
    class Meta(AbstractCountryListModel.Meta):
        managed = False


# ZIP CODE
class AbstractZipCodeListModel(models.Model):
    """
    READ ONLY MODEL
    """
...
class ZipCodeListModel(AbstractZipCodeListModel):
    class Meta(AbstractZipCodeListModel.Meta):
        managed = False
...
class AbstractUserCreditCardModel(models.Model):
    ...
    card_nickname = models.CharField(max_length=50, db_column='BillingCompanyName', blank=True,
    ...
    payment_profile_id = models.CharField(max_length=50, db_column='CustomerPaymentProfileID', blank=True)
    ...
    def card_expire(self):
        return "%s/%s" % (
            self.card_expire_month, self.card_expire_year)

    @property
    def is_expired(self):
        if datetime.date.today() > datetime.date(
                int(self.card_expire_year),
                int(self.card_expire_month),
                calendar.monthrange(int(self.card_expire_year),
                                    int(self.card_expire_month))[1]):
            return True
        else:
            return False
...
class UserCreditCardModel(AbstractUserCreditCardModel):
    user = models.ForeignKey('meta_db.UserModel', db_column='TBCustomer_ID', max_length=8, to_field='customer_id', related_name='credit_cards', on_delete=models.DO_NOTHING)
