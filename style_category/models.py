...
class AbstractGrandCategoryModel(models.Model):
    ...
    name = models.CharField(db_column='group_name', max_length=50, help_text='grouped for showing.')
...
class AbstractStyleCategoryMasterModelManager(models.Manager):
    def all_active(self, **kwargs):
        return super(AbstractStyleCategoryMasterModelManager, self).get_queryset().filter(is_active=True, **kwargs)
...
class AbstractStyleCategoryMasterModel(models.Model):
    ...
    """
    id = models.AutoField(db_column='TBStyleNo_OS_Category_Master_ID', primary_key=True)
    name = models.CharField(db_column='CategoryMasterName', max_length=50)
    description = models.TextField(db_column='Description', blank=True, null=True)
    short_description = models.TextField(db_column='Description50', blank=True, null=True,
                                         help_text='short description')
    ordering = models.IntegerField(db_column='DisplayOrder', default=0)
    display_group = models.CharField(db_column='DisplayGroup', max_length=50,
                                     help_text='grouped for showing.')
    display_group_order = models.IntegerField(db_column='DisplayGroupOrder',
                                              help_text='ordering of grouped for showing')
    slug_name = models.CharField(db_column='url', max_length=50, unique=True,
                                 help_text='slug style name')
    is_active = models.BooleanField(db_column='Active', default=True)
    # main_top_category = models.SmallIntegerField(db_column='mainTopCategory')
...
class AbstractStyleCategorySubModelManager(models.Manager):
    def all_active(self, **kwargs):
        return super(AbstractStyleCategorySubModelManager, self).get_queryset().filter(is_active=True, **kwargs)
...
class AbstractStyleCategorySubModel(models.Model):
    ...
    DO NOT DELETE. JUST MAKE IT INACTIVE.
    """
    id = models.AutoField(db_column='TBStyleNo_OS_Category_Sub_ID', primary_key=True)
    name = models.CharField(db_column='CategorySubName', max_length=50)
    slug_name = models.CharField(db_column='url', max_length=50,
                                 help_text='slug name')
    short_description = models.TextField(db_column='Description50', blank=True, null=True,
                                         help_text='short description')
...
class StyleCategorySubModel(AbstractStyleCategorySubModel):
    master_category = models.ForeignKey('StyleCategoryMasterModel',
                                        db_column='TBStyleNo_OS_Category_Master_ID', related_name='sub_categories',
                                        blank=True, null=True, on_delete=models.DO_NOTHING)

    def validate_unique(self, exclude=None):
        if self.__class__.objects.filter(master_category=self.master_category,
                                         slug_name=self.slug_name).exists():
            raise ValidationError({NON_FIELD_ERRORS: ['Sub category already exists.',],})
