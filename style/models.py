...
class AbstractSizeChartQuantityModel(models.Model):
    ...
    description = models.CharField(max_length=50, db_column='dPackNoDescription', blank=True)
    ...
    @property
    def qty_list(self):
        _tmp_list = ['d1p', 'd2p', 'd3p', 'd4p', 'd5p', 'd6p', 'd7p',
                     'd8p', 'd9p', 'd10p', 'd11p', 'd12p', 'd13p']
        _qty_list = []
        for i in _tmp_list:
...
class AbstractSizeChartLabelModel(models.Model):
    ...
    description = models.CharField(max_length=50, db_column='dSizeDescription', blank=True)
    ...
    @property
    def size_list(self):
        _tmp_list = ['d1s', 'd2s', 'd3s', 'd4s', 'd5s', 'd6s', 'd7s',
                     'd8s', 'd9s', 'd10s', 'd11s', 'd12s', 'd13s']
...
class AbstractShoeSizeChartModel(models.Model):
    ...
    name = models.CharField(max_length=50, db_column='ShoeSizeName', blank=True)
    ...
    def shoe_size_chart_label(self):
        return ['5', '5.5', '6', '6.5', '7', '7.5', '8', '8.5', '9', '9.5', '10', '10.5', '11', '11.5', '12', '12.5']

    @property
    def shoe_size_chart(self):
        _tmp_list = ['s5', 's5h', 's6', 's6h', 's7', 's7h', 's8', 's8h', 's9', 's9h', 's10',
                     's10h', 's11', 's11h', 's12', 's12h']
        _size_list = []
        for i in _tmp_list:
            try:
    ...
    def shoe_size_chart(self, values):
        ...
        if len(values) != len(self.shoe_size_chart_label):
            raise ValueError('shoe size run list length is %s' % (len(self.shoe_size_chart_label),))
        for i, v in zip(_tmp_list, values):
            setattr(self, i, v)
...
class ShoeSizeChartModel(AbstractShoeSizeChartModel):
    brand = models.ForeignKey('meta_db.BrandModel', db_column='TBVendor_ID', blank=True, related_name='brand_all_shoe_size', on_delete=models.DO_NOTHING)

    def validate_unique(self, exclude=None):
        if self.__class__.objects.filter(name=self.name, brand=self.brand).exists():
            raise ValidationError({NON_FIELD_ERRORS: ['Shoe size chart name already exists.',],})
...
class AbstractSelectedShoeSizeChartModel(models.Model):
    ...
    """
    id = models.AutoField(db_column='TBVendorShoeSizeSelect_ID', primary_key=True)
...
class SelectedShoeSizeChartModel(AbstractSelectedShoeSizeChartModel):
    item = models.ForeignKey('ProductModel', db_column='TBItem_ID', related_name='shoe_size', on_delete=models.DO_NOTHING)
    shoe_size = models.ForeignKey('ShoeSizeChartModel', db_column='TBVendorShoeSize_ID', on_delete=models.DO_NOTHING)

    def validate_unique(self, *args, **kwargs):
        if self.__class__.objects.filter(item=self.item, shoe_size=self.shoe_size).exists():
            raise ValidationError({NON_FIELD_ERRORS: ['Shoe size chart already exists.',],})
...
class AbstractStyleCollectionModel(models.Model):
    ...
    """
    id = models.AutoField(db_column='TBStyleNo_OS_Collection_ID', primary_key=True)
...
class AbstractProductInfoStyleModel(models.Model):
    ...
    name = models.CharField(db_column='StyleName', max_length=50, blank=True, null=True)
    description = models.CharField(db_column='Description', max_length=50, blank=True, null=True)
    is_active = models.BooleanField(null=True,db_column='Active')
...
class AbstractProductInfoStyleSelectedModel(models.Model):
    ...
    """
    id = models.AutoField(db_column='TBStyleNo_OS_StyleSelect_ID', primary_key=True)
...
class ProductInfoStyleSelectedModel(AbstractProductInfoStyleSelectedModel):
    item = models.ForeignKey('meta_db.ProductModel', db_column='TBItem_ID', max_length=8, related_name='pi_style_item', on_delete=models.DO_NOTHING)
    style = models.ForeignKey('ProductInfoStyleModel', db_column='TBStyleNo_OS_Style_ID', related_name='pi_style', on_delete=models.DO_NOTHING)
...
class AbstractProductInfoPatternModel(models.Model):
    id = models.AutoField(db_column='TBStyleNo_OS_Pattern_ID', primary_key=True)
    name = models.CharField(db_column='PatternName', max_length=50, blank=True, null=True)
    description = models.CharField(db_column='Description', max_length=50, blank=True, null=True)
    is_active = models.BooleanField(null=True,db_column='Active')
...
class AbstractProductInfoPatternSelectedModel(models.Model):
    id = models.AutoField(db_column='TBStyleNo_OS_PatternSelect_ID', primary_key=True)
    # nadd = models.CharField(db_column='nAdd', max_length=1, blank=True, null=True)
    # ndelete = models.CharField(db_column='nDelete', max_length=1, blank=True, null=True)
...
class ProductInfoPatternSelectedModel(AbstractProductInfoPatternSelectedModel):
    item = models.ForeignKey('ProductModel', db_column='TBItem_ID', max_length=8, related_name='pi_pattern_item', on_delete=models.DO_NOTHING)
    pattern = models.ForeignKey('ProductInfoPatternModel', db_column='TBStyleNo_OS_Pattern_ID', related_name='pi_pattern', on_delete=models.DO_NOTHING)
...
class AbstractProductInfoSleeveModel(models.Model):
    ...
    name = models.CharField(db_column='SleeveName', max_length=50, blank=True, null=True)
    description = models.CharField(db_column='Description', max_length=50, blank=True, null=True)
    is_active = models.BooleanField(null=True,db_column='Active')
...
class AbstractProductInfoSleeveSelectedModel(models.Model):
    id = models.AutoField(db_column='TBStyleNo_OS_SleeveSelect_ID', primary_key=True)
    # nadd = models.CharField(db_column='nAdd', max_length=1, blank=True, null=True)
    # ndelete = models.CharField(db_column='nDelete', max_length=1, blank=True, null=True)
...
class ProductInfoSleeveSelectedModel(AbstractProductInfoSleeveSelectedModel):
    item = models.ForeignKey('ProductModel', db_column='TBItem_ID', max_length=8, related_name='pi_sleeve_item', on_delete=models.DO_NOTHING)
    sleeve = models.ForeignKey('ProductInfoSleeveModel', db_column='TBStyleNo_OS_Sleeve_ID', related_name='pi_sleeve', on_delete=models.DO_NOTHING)
...
class AbstractProductModelManager(models.Manager):
    def all_active(self):
        return super(AbstractProductModelManager, self).get_queryset().filter(_is_active='Y')


class AbstractProductModel(models.Model):
    id = models.CharField(max_length=8, db_column='TBItem_ID', primary_key=True)
    style_name = models.CharField(max_length=40, db_column='nStyleName', blank=True)
    grouped_item_code = models.CharField(max_length=12, db_column='nVendorBarItem', blank=True)
    os_style_number = models.CharField(max_length=30, db_column='nOurStyleNo', blank=True,
                                       help_text='style number for os')
    brand_style_number = models.CharField(max_length=30, db_column='nVendorStyleNo', blank=True,
    ...
    _is_active = models.CharField(max_length=1, db_column='nActive', default='Y')
    # _is_brand_active = models.CharField(max_length=1, db_column='nVendorActive', blank=True)
    _is_prepare = models.CharField(max_length=1, db_column='nPrepare', default='Y')
    purchase_price = models.DecimalField(decimal_places=2, null=True, max_digits=19, db_column='nPurchasePrice',
                                         blank=True, help_text='purchased priced from brand')
    os_price = models.DecimalField(decimal_places=2, null=True, max_digits=19, db_column='nPrice2', blank=True)
    _is_sale = models.CharField(max_length=1, db_column='nOnSale', default='N')
    os_sale_price = models.DecimalField(decimal_places=2, null=True, max_digits=19, db_column='nSalePrice2', blank=True)
    brand_shop_price = models.DecimalField(decimal_places=2, max_digits=19, db_column='nPrice1', default=0.00,
                                           help_text='brand shop price')
    brand_shop_sale_price = models.DecimalField(decimal_places=2, max_digits=19, db_column='nSalePrice1', default=0.00,
                                                help_text='brand shop sale price')
    sub_invoice_sale_price = models.DecimalField(decimal_places=2, null=True, max_digits=19,
                                                 db_column='nSalePrice', blank=True,
                                                 help_text='if item is sale, input this number to sub invoice purchase price')
    created = models.DateTimeField(null=True, db_column='nDate', blank=True, auto_now_add=True)
    updated = models.DateTimeField(null=True, db_column='nModifyDate', blank=True, auto_now=True)
    ...
    _is_restock = models.CharField(max_length=1, db_column='nReStock', default='N',
                                   help_text='restock item')
    restock_date = models.DateTimeField(null=True, db_column='nReStockDate', blank=True)
    _is_preorder = models.CharField(max_length=1, db_column='nPreOrder', default='N')
    preorder_available_date = models.DateTimeField(null=True, db_column='nPreOrderAvailableDate', blank=True)
    is_brand_only = models.BooleanField(db_column='is_brand_only', default=False,
                                        help_text='only sales in brand shop.if true, is_active should be false')
    is_brand_only_active = models.BooleanField(db_column='is_brand_only_active', default=False,
                                               help_text='is brand only item active')
    pictures1 = models.CharField(max_length=100, db_column='PictureS1', blank=True)
    pictures2 = models.CharField(max_length=100, db_column='PictureS2', blank=True)
    pictures3 = models.CharField(max_length=100, db_column='PictureS3', blank=True)
    pictures4 = models.CharField(max_length=100, db_column='PictureS4', blank=True)
    pictures5 = models.CharField(max_length=100, db_column='PictureS5', blank=True)
    pictures6 = models.CharField(max_length=100, db_column='PictureS6', blank=True)
    pictures7 = models.CharField(max_length=100, db_column='PictureS7', blank=True)
    pictures8 = models.CharField(max_length=100, db_column='PictureS8', blank=True)
    pictures9 = models.CharField(max_length=100, db_column='PictureS9', blank=True)
    picturer1 = models.CharField(max_length=100, db_column='PictureR1', blank=True)
    picturer2 = models.CharField(max_length=100, db_column='PictureR2', blank=True)
    picturer3 = models.CharField(max_length=100, db_column='PictureR3', blank=True)
    picturer4 = models.CharField(max_length=100, db_column='PictureR4', blank=True)
    picturer5 = models.CharField(max_length=100, db_column='PictureR5', blank=True)
    picturer6 = models.CharField(max_length=100, db_column='PictureR6', blank=True)
    picturer7 = models.CharField(max_length=100, db_column='PictureR7', blank=True)
    picturer8 = models.CharField(max_length=100, db_column='PictureR8', blank=True)
    picturer9 = models.CharField(max_length=100, db_column='PictureR9', blank=True)
    picture1 = models.CharField(max_length=100, db_column='Picture1', blank=True)
    picture2 = models.CharField(max_length=100, db_column='Picture2', blank=True)
    picture3 = models.CharField(max_length=100, db_column='Picture3', blank=True)
    picture4 = models.CharField(max_length=100, db_column='Picture4', blank=True)
    picture5 = models.CharField(max_length=100, db_column='Picture5', blank=True)
    picture6 = models.CharField(max_length=100, db_column='Picture6', blank=True)
    picture7 = models.CharField(max_length=100, db_column='Picture7', blank=True)
    picture8 = models.CharField(max_length=100, db_column='Picture8', blank=True)
    picture9 = models.CharField(max_length=100, db_column='Picture9', blank=True)
    picturez1 = models.CharField(max_length=100, db_column='PictureZ1', blank=True)
    picturez2 = models.CharField(max_length=100, db_column='PictureZ2', blank=True)
    picturez3 = models.CharField(max_length=100, db_column='PictureZ3', blank=True)
    picturez4 = models.CharField(max_length=100, db_column='PictureZ4', blank=True)
    picturez5 = models.CharField(max_length=100, db_column='PictureZ5', blank=True)
    picturez6 = models.CharField(max_length=100, db_column='PictureZ6', blank=True)
    picturez7 = models.CharField(max_length=100, db_column='PictureZ7', blank=True)
    picturez8 = models.CharField(max_length=100, db_column='PictureZ8', blank=True)
    picturez9 = models.CharField(max_length=100, db_column='PictureZ9', blank=True)
    picturev1 = models.CharField(max_length=100, db_column='PictureV1', blank=True)
    picturev2 = models.CharField(max_length=100, db_column='PictureV2', blank=True)
    picturev3 = models.CharField(max_length=100, db_column='PictureV3', blank=True)
    picturev4 = models.CharField(max_length=100, db_column='PictureV4', blank=True)
    picturev5 = models.CharField(max_length=100, db_column='PictureV5', blank=True)
    picturev6 = models.CharField(max_length=100, db_column='PictureV6', blank=True)
    picturev7 = models.CharField(max_length=100, db_column='PictureV7', blank=True)
    picturev8 = models.CharField(max_length=100, db_column='PictureV8', blank=True)
    picturev9 = models.CharField(max_length=100, db_column='PictureV9', blank=True)
    popular_point = models.SmallIntegerField(db_column='popular_point', help_text='popular point')
    view_point = models.SmallIntegerField(db_column='view_point', help_text='most viewed')
    sales_point = models.SmallIntegerField(db_column='sales_point', help_text='most sales')
    _is_hidden = models.CharField(db_column='nHidden', blank=True, max_length=1, default='N',
    ...
    def pictures(self):
        _length = range(1, 10)
        return dict(
            tiny=[getattr(self, "pictures%s" % i) for i in _length if getattr(self, "picture%s" % i)],
            medium=[getattr(self, "picture%s" % i) for i in _length if getattr(self, "picture%s" % i)],
            large=[getattr(self, "picturez%s" % i) for i in _length if getattr(self, "picture%s" % i)],
        )
    ...
    def pictures(self, values):
        ...
        if not type(dict()) is type(values) or not any(map(lambda l: l in list(_scaffold.keys()), list(values.keys()))):
...
class ProductModel(AbstractProductModel):
    ...
    brand_category = models.ForeignKey('meta_db.BrandCategoryModel', db_column='TBVendorOwnCategory_ID',
                                       help_text='brand own categories', on_delete=models.DO_NOTHING)
    os_category_master = models.ForeignKey('meta_db.StyleCategoryMasterModel', db_column='TBStyleNo_OS_Category_Master_ID', on_delete=models.DO_NOTHING)
    os_category_sub = models.ForeignKey('meta_db.StyleCategorySubModel', db_column='TBStyleNo_OS_Category_Sub_ID', on_delete=models.DO_NOTHING)
    os_collection = models.ForeignKey('StyleCollectionModel', db_column='TBStyleNo_OS_Collection_ID',
                                      help_text='os style collection id', on_delete=models.DO_NOTHING)
    size_chart = models.ForeignKey('SizeChartLabelModel', db_column='TBSizeChart_ID', on_delete=models.DO_NOTHING)
    pack = models.ForeignKey('SizeChartQuantityModel', db_column='TBPackNo_ID', on_delete=models.DO_NOTHING)

    @property
    def grouped_items(self):
        return ProductModel.objects.filter(grouped_item_code=self.grouped_item_code, brand=self.brand)
    ...
    def colors(self):
        ...
        """
        return ColorSelectedModel.objects.all_active().filter(item__grouped_item_code=self.grouped_item_code,
                                                              item__brand=self.brand)
    @property
    def is_shoes(self):
        return True if self.os_category_master_id == 15 and self.shoe_size.all() else False
...
class AbstractColorModelManager(models.Manager):
    def all_active(self, **kwargs):
        return super(AbstractColorModelManager, self).get_queryset().filter(is_active=True, **kwargs)


class AbstractColorModel(models.Model):
    id = models.CharField(max_length=10, db_column='TBColor_ID', primary_key=True)
    name = models.CharField(max_length=30, db_column='cColorName')
    description = models.CharField(max_length=50, db_column='cColorDescription', blank=True)
    is_active = models.BooleanField(db_column='cActive', default=True, help_text='T/F')
    _is_added = models.CharField(max_length=1, db_column='cAdd', default='Y', help_text='for ARA Y/N')
    _is_deleted = models.CharField(max_length=1, db_column='cDelete', default='N', help_text='for ARA Y/N')
...
class AbstractColorSelectedModelManager(models.Manager):
    def all_active(self, **kwargs):
        return super(AbstractColorSelectedModelManager, self).get_queryset().filter(_is_active='Y', **kwargs)


class AbstractColorSelectedModel(models.Model):
    id = models.AutoField(db_column='TBColorSelect_ID', primary_key=True)
    _is_active = models.CharField(max_length=1, db_column='Active', default='Y', help_text='active Y/N')
    updated_date = models.DateTimeField(db_column='nUpdatedDate', auto_now=True)
    image_link = models.CharField(max_length=50, db_column='ImageLink', blank=True)
...
class ColorSelectedModel(AbstractColorSelectedModel):
    item = models.ForeignKey('ProductModel', db_column='TBItem_ID', related_name='only_colors',
                             help_text='colors only belong to an item. Excluding set bundle or set docking ', on_delete=models.DO_NOTHING)
