...
class AbstractFulfillmentModel(models.Model):
    ...
    free_shipping_amt = models.DecimalField(db_column='FreeShippingAmount',
                                            max_digits=18,
                                            decimal_places=0,
                                            blank=True,
                                            null=True)
    ...
    min_discount_amt = models.DecimalField(db_column='AmountDiscountMinimumAmount',
                                           ...
                                           blank=True,
                                           null=True)
    min_order_amt = models.DecimalField(db_column='InvMinAmount',
                                        max_digits=18,
                                        decimal_places=0,
                                        blank=True,
                                        null=True)
    ...
    def full_address(self):
        return '{}, {}, {} {}'.format(
            '{} {}'.format(self.address1, self.address2) if self.address2 else '{}'.format(self.address1),
...
class AbstractBrandCategoryModel(models.Model):
    ...
    sorting_order = models.IntegerField(db_column='SortingNoOnBrand',
                                        blank=True,
                                        null=True,
...
class BrandCategoryModel(AbstractBrandCategoryModel):
    ...
    os_master_category = models.ForeignKey('meta_db.StyleCategoryMasterModel',
                                           ...
                                           blank=True,
                                           on_delete=models.DO_NOTHING,
                                           help_text='matched it from os category')
    os_sub_category = models.ForeignKey('meta_db.StyleCategorySubModel',
...
class AbstractBrandModel(models.Model):
    ...
    contact = models.CharField(max_length=30, db_column='VDContact', blank=True, help_text='represent name')
    ...
    brand_domain = models.CharField(max_length=100,
                                    db_column='TBVendor_Domain',
                                    blank=True,
                                    help_text='brand own website url')
    # contact_us = models.TextField(db_column='VDContactUs', blank=True) #This field type is a guess.
    email = models.CharField(max_length=50, db_column='VDVendorEmail', blank=True, help_text='brand represent email')
    ...
    min_order_amt = models.DecimalField(decimal_places=4,
                                        null=True,
                                        max_digits=19,
                                        db_column='OSminOrderAMT',
                                        blank=True,
                                        help_text='minimum order at orangeshine.com')
    brand_segment = models.CharField(max_length=50,
                                     db_column='VDCategory',
                                     blank=True,
                                     help_text='brand products segment. just text')
    copy_prevention = models.BooleanField(db_column='copyprevention', default=1, help_text='hide items for anonymous')
    brand_description = models.TextField(db_column='OSDescription', blank=True)
    brand_price_range = models.IntegerField(null=True,
                                            db_column='VDPriceLevel',
                                            blank=True,
                                            help_text='brand price range')
    manufacture = models.CharField(max_length=50, db_column='VDMadeIn', blank=True, help_text='manufacture location')
    _is_paid_home = models.CharField(max_length=1, db_column='VDPaidDetail', blank=True, help_text='paid brand\'s home')
    brand_head_title = models.CharField(max_length=100,
                                        db_column='VDH1Tag',
                                        blank=True,
                                        help_text='brand title h1 table title')

    _joined_os_free_shipping = models.CharField(max_length=1,
                                                db_column='VDFSB',
                                                blank=True,
                                                help_text='join free shipping promotion on OS')
    os_commission = models.FloatField(null=True,
                                      db_column='VDDiscountFee',
                                      blank=True,
                                      default=0.0,
                                      help_text='OS Commission Rate')
    merchant_fee = models.FloatField(null=True,
                                     db_column='VDMerchantFee',
                                     blank=True,
                                     default=0.0,
                                     help_text='Credit Card & Paypal Fee Rate')
    international_shipping = models.BooleanField(db_column='internationalShipping', default=True)
    ...
    @property
    def size_chart_list(self):
        from apps.brand.handlers.brand_settings_handler import BrandSettingsHandler
        return BrandSettingsHandler().get_brand_settings(brand_id=self.id, settings_type='size_chart')

    @property
    def return_policy(self):
        from apps.brand.handlers.brand_settings_handler import BrandSettingsHandler
        return BrandSettingsHandler().get_brand_settings(brand_id=self.id, settings_type='return_policy')

    @property
    def full_address(self):
        return '{}, {}, {} {}'.format(
            '{} {}'.format(self.address, self.address2) if self.address2 else '{}'.format(self.address),
            self.city, self.state, self.zipcode
        )
...
class BrandModel(AbstractBrandModel):
    fulfillment = models.ForeignKey('FulfillmentModel',
                                    db_column='ShippedFrom',
                                    to_field='fulfillment',
                                    on_delete=models.DO_NOTHING,
                                    blank=True)
    style_collection = models.ForeignKey('meta_db.StyleCollectionModel',
