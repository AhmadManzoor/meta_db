...
from meta_db.brand.models import FulfillmentModel
from meta_db.invoice.models import VoidHistoryModel
...
class AbstractMasterPOModel(AbstractMetaMasterInvoiceModel):
    ...
    ordered_date = models.DateTimeField(db_column='OrderDate',
                                        auto_now_add=True)
    ...
    order_amount = models.DecimalField(
        ...
        blank=True,
        default=0.00,
        help_text='for original order amount, read-only')
    is_void = models.BooleanField(db_column='POVoid',
                                  default=False,
                                  null=False)
    ...
    # COMMENTS
    po_memo = models.CharField(max_length=3000,
                               db_column='PONote',
                               blank=True,
                               default='')
    brand_comment = models.CharField(max_length=3000,
                                     db_column='CommentsByBrand',
                                     blank=True,
                                     help_text='comments by brand on PO')
    ...
    void_reason_code = models.CharField(db_column='VoidReasonCode',
                                        ...
                                        blank=True)
    # PROMOTION DISCOUNT
    po_star = models.SmallIntegerField(
        db_column='PORating',
        blank=True,
        null=True,
        help_text='for fraud detection algorithm')
    os_commission = models.FloatField(db_column='OSCommission',
                                      blank=True,
                                      null=False,
                                      default=0.0,
                                      help_text='OS Commission Rate')
    merchant_fee = models.FloatField(db_column='MerchantFee',
                                     blank=True,
                                     null=False,
                                     default=0.0,
    ...
    def parent_master_invoice(self):
        ...
        try:
            parent_master = POMasterInvoiceMapModel.objects.get(
                po__id=self.id, map_type="BO").invoice_master
    ...
    def child_master_invoices(self):
        ...
        try:
            ...
            for _map in POMasterInvoiceMapModel.objects.filter(
                    po__id=self.id, map_type__in=['PO', 'BPO']):
    ...
    def _get_unique_po_number(self):
        ...
        try:
            while True:
                _po_id = MasterPOModel.objects.get(
                    **{'po_number': _candidate_id})
                _key += 1
                _candidate_id = "PO-WB%s" % _key
...
class MasterPOModel(AbstractMasterPOModel):
    user = models.ForeignKey('meta_db.UserModel',
                             ...
                             related_name='master_po',
                             on_delete=models.DO_NOTHING)
    po_status = models.ForeignKey('meta_db.MasterPOStatusModel',
                                  db_column='TBPOStatus_ID',
                                  default=1,
                                  on_delete=models.DO_NOTHING)
    ...
    payment_method = models.ForeignKey(
        ...
        help_text='FK. payment method id. TBSaleMethod table',
        on_delete=models.DO_NOTHING)
    ...
    def get_status_name(self):
        PO_STATUS = settings.ORDER_STATUS['pre-order']
        try:
    ...
    def fulfillment_by(self):
        try:
            return self.fulfilled_by.fulfillment
    ...
    def shipped_from(self):
        """
        remove prefix ('CM_') from chermuse fulfillment
        """
        fulfillment_name = ''
        fulfillment = self.fulfillment_by
        if fulfillment:
            if fulfillment.startswith('CM_'):
                fulfillment_name = fulfillment[3:]
            else:
                fulfillment_name = fulfillment
    ...
    def order_status(self):
        _STATUS = settings.ORDER_STATUS['pre-order']
        return _STATUS[self.po_status.id]

    @property
    def auth_form_required(self):
        if int(
                self.po_status_id
        ) == 8 and self.payment_method_id == 'WH1' and self.po_star <= 2:
...
class AbstractPOLineModel(models.Model):
    ...
    line_id = models.CharField(
        max_length=17,
        db_column='POLineNumber',
        null=False,
        unique=True,
        help_text='automatically and uniquely generated as PLnn..nn')
    created_date = models.DateTimeField(null=True,
                                        db_column='CreateDate',
                                        auto_now_add=True)
    updated_date = models.DateTimeField(db_column='UpdateDate',
                                        auto_now_add=True)
    ...
    total_item_qty = models.SmallIntegerField(db_column='TotalItemQty',
                                              null=False)
    # PRICE VALUES
    ...
    discounted_price = models.DecimalField(decimal_places=4,
                                           max_digits=19,
                                           null=True,
                                           blank=True,
                                           db_column='DiscountedPrice',
                                           default=0.00)
    ...
    preorder_available_date = models.DateTimeField(
        null=True, blank=True, db_column='nPreOrderAvailableDate')
...
class POLineModel(AbstractPOLineModel):
    master_order = models.ForeignKey('MasterPOModel',
                                     db_column='PONumber',
                                     to_field='po_number',
    ...
    shoe_size = models.ForeignKey('meta_db.ShoeSizeChartModel',
                                  ...
                                  default=0,
                                  on_delete=models.DO_NOTHING)
    line_status = models.ForeignKey('POLineStatusModel',
                                    db_column='TBPOLineStatus_ID',
                                    default=1,
                                    on_delete=models.DO_NOTHING)
                    ...
                    self.line_id = 'PL-{}-{}'.format(
                        int(time.time()), ''.join(
                            random.choice(string.ascii_uppercase +
                                          string.digits) for i in range(3)))
                    self.save(*args, **kwargs)
                else:
                    raise e
        else:
...
class POLineStatusManager(models.Manager):
    # TODO: abstract and meta
    def all_active(self):
        return super(POLineStatusManager,
                     self).get_queryset().filter(is_active=True)
...
class AbstractPOMasterInvoiceMapModel(models.Model):
    ...
    invoice_master = models.ForeignKey('MasterInvoiceModel',
                                       ...
                                       on_delete=models.PROTECT)
    map_type = models.CharField(db_column='MapType',
                                max_length=5,
                                default='PO',
