...
class AbstractCheckoutStepTraceModel(models.Model):
    ...
    """
    id = models.TextField(db_column='TBCheckoutHistory_ID',
                          primary_key=True,
                          help_text='uuid',
                          unique=True)
    customer_id = models.CharField(db_column='TBCustomer_ID', max_length=8)
    step = models.IntegerField(db_column='Step')
    marked_date = models.DateTimeField(db_column='InputDate',
                                       auto_now_add=True)
    cart_amount = models.DecimalField(db_column='CartAMT',
                                      max_digits=19,
                                      decimal_places=4,
                                      default=0.00)
